...
public class RedBlackBST {
    ...
    private static final Logger logger = Logger.getLogger(RedBlackBST.class.getName());
    ...
    private void draw(Node currentNode, String indent, boolean last) {
        if (currentNode != null) {
            ...
            draw(currentNode.left, indent, false);
            draw(currentNode.right, indent, true);
        }
    }
    ...
    public static void main(String[] args) {
        ...
        logger.log(Level.INFO, "Введите количество символов для построение красно-черного дерева: ");
        int amount = input.nextInt();

        for (int i = 0; i < amount; i++) {
            logger.log(Level.INFO, "Введите {0} символ: ", i + 1);
            char key = input.next().charAt(0);
            tree.addNode(key);
        }

        tree.draw();
        logger.log(Level.INFO, "Дерево создано!{0}", tree.result);
